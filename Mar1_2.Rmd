...
```{r}
predict.lm(model = fitness_lm, newdata = new_times)
```

We can tell it to add intervals as well:

```{r}
predict.lm(model = fitness_lm, newdata = new_times, interval="prediction")
```

```{r}
predict.lm(model = fitness_lm, newdata = new_times, interval="confidence")
```
